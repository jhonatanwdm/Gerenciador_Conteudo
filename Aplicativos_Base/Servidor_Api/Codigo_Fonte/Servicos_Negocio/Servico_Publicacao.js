...
exports.ServicoPublicacao = void 0;
...
class ServicoPublicacao {
    static async listarTodas(filtros) {
        const publicacoes = await banco_dados_1.default.publicacao_Acao.findMany({
            where: {
        ...
        return publicacoes.map((pub) => {
            ...
            const processando = pub.destinos.filter((d) => ['QUEUED', 'VALIDATING', 'PROCESSING', 'UPLOADING', 'PUBLISHING'].includes(d.status)).length;
    ...
    static async obterPorId(id) {
        const pub = await banco_dados_1.default.publicacao_Acao.findUnique({
            where: { id },
            include: {
                conteudo: true,
                campanha: true,
    ...
    static async criarPublicacao(dados) {
        const conteudo = await banco_dados_1.default.conteudo_Base.findUnique({
        ...
        const publicacao = await banco_dados_1.default.publicacao_Acao.create({
            data: {
        ...
        for (const d of dados.destinos) {
            ...
            const destino = await banco_dados_1.default.destino_Publicacao.create({
                data: {
    ...
    static async processarDestinoIndividual(destinoId) {
        const destino = await banco_dados_1.default.destino_Publicacao.findUnique({
            where: { id: destinoId },
            include: {
                conta_social: {
        ...
        if (!destino || !destino.conta_social)
            return;
        // Atualiza status para PROCESSING
        await banco_dados_1.default.destino_Publicacao.update({
            where: { id: destinoId },
        ...
        try {
            ...
            if (destino.conta_social.credencial_oauth?.token_acesso_cifrado) {
                tokenAcesso = Servico_Criptografia_js_1.ServicoCriptografia.decifrar(destino.conta_social.credencial_oauth.token_acesso_cifrado);
            }
            // Validação de regras e capacidades
            if (destino.conta_social.capacidade_conta) {
                nucleo_plataformas_1.ResolvedorCapacidades.validarCapacidade({
            ...
            const metricasIniciais = await adaptador.obterMetricas(resultado.idExterno || 'mock', tokenAcesso);
            await banco_dados_1.default.instantaneo_Metricas.create({
                data: {
            ...
            await Servico_Registro_js_1.ServicoRegistro.registrarSistema({
                nivel: 'ERROR',
    ...
    static async atualizarStatusGlobal(publicacaoId) {
        const destinos = await banco_dados_1.default.destino_Publicacao.findMany({
        ...
        await banco_dados_1.default.publicacao_Acao.update({
            where: { id: publicacaoId },
            data: { status_global: statusGlobal },
        });
    }
    static async tentarNovamenteDestino(destinoId) {
        const destino = await banco_dados_1.default.destino_Publicacao.findUnique({
            where: { id: destinoId },
        });
        if (!destino) {
    ...
    static async repostarPublicacao(dados) {
        const publicacaoOrigem = await banco_dados_1.default.publicacao_Acao.findUnique({
        ...
        await banco_dados_1.default.relacao_Publicacao.create({
            data: {
...
exports.ServicoPublicacao = ServicoPublicacao;
//# sourceMappingURL=Servico_Publicacao.js.map